...
object sqloutput { module =>
  ...
  object SQLOutputOp {
    ...
    trait Visitor[F[_]] extends (SQLOutputOp ~> F) {
      ...
      def delay[A](a: () => A): F[A]
      def suspend[A](a: () => SQLOutputIO[A]): F[A]
    ...
    case class Suspend[A](a: () => SQLOutputIO[A]) extends SQLOutputOp[A] {
    ...
    case class RaiseError[A](t: Throwable) extends SQLOutputOp[A] {
  ...
  def suspend[A](a: => SQLOutputIO[A]): SQLOutputIO[A] = FF.liftF(Suspend(() => a))
  ...
  def raiseError[A](err: Throwable): SQLOutputIO[A] = FF.liftF(RaiseError(err))
  ...
  implicit val AsyncSQLOutputIO: Async[SQLOutputIO] =
    new Async[SQLOutputIO] {
      ...
      def suspend[A](thunk: => SQLOutputIO[A]): SQLOutputIO[A] = module.suspend(thunk)
    }
